...
import {
  ...
  fmtMod, modifier, getRace, rollAbility,
} from "@/lib/dnd-data";
...
function AbilitiesStep() {
  const { abilities, abilityMethod, raceId, setAbility, setField } = useCharacter();
  ...
  const pointsSpent = useMemo(() => {
    ...
    return ABILITIES.reduce((sum, a) => sum + (POINT_BUY_COST[abilities[a.key]] ?? 0), 0);
  }, [abilities, abilityMethod]);
  ...
  const chooseMethod = (m: typeof METHODS[number]["id"]) => {
    ...
    if (m === "standard") {
      ...
      ABILITIES.forEach((a) => setAbility(a.key, 8));
    } else if (m === "random") {
      ABILITIES.forEach((a) => setAbility(a.key, rollAbility()));
    }
  };

  const rerollAll = () => ABILITIES.forEach((a) => setAbility(a.key, rollAbility()));
    ...
    <section>
      <header className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Escolha o método de distribuição. Bônus raciais somam ao final.
        </p>
      </header>
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border/60 bg-secondary/30 px-4 py-3">
          ...
          {abilityMethod === "random" && (
            <>
              <span className="text-xs uppercase tracking-widest text-muted-foreground">
                4d6, descarte o menor. Total: <span className="font-mono text-primary">
          ...
          {abilityMethod === "pointbuy" && (
            <span className="text-xs uppercase tracking-widest text-muted-foreground">
              Pontos: <span className={`font-mono ${pointsLeft < 0 ? "text-destructive" : "text-primary"}`}>
                {pointsLeft}
              </span> / {POINT_BUY_TOTAL} — valores 8–15
            </span>
          )}
        </div>
                  ...
                  {abilityMethod === "pointbuy" && (
                    <div className="flex flex-1 items-center gap-2">
                      <button type="button"
                        ...
                        className="rounded-md border border-border bg-secondary/60 px-3 py-2 text-primary hover:border-primary">−</button>
