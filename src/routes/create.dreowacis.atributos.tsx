...
function AbilitiesStep() {
  const { abilities, abilityMethod, kingdomId, setAbility, setField } = useDreowacis();
    ...
    <section>
      <header className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Modificador = (valor − 10) ÷ 2, arredondando para baixo.
        </p>
      </header>
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {ABILITIES.map((a) => {
            ...
            const bonus = kingdom?.bonusAbility === a.key ? 1 : 0;
              ...
              <div key={a.key} className="rune-panel rounded-xl p-5">
                ...
                <div className="mt-3 flex items-end gap-3">
                  ...
                  <div className="text-right">
                    <p className="font-display text-3xl text-primary text-glow-soft">{fmtMod(mod)}</p>
                    <p className="text-[10px] uppercase tracking-widest text-muted-foreground">mod</p>
                  </div>
                </div>
                <p className="mt-2 text-[11px] text-muted-foreground">
                  {base} {bonus > 0 ? `+ ${bonus} (${kingdom?.name})` : ""} ={" "}
